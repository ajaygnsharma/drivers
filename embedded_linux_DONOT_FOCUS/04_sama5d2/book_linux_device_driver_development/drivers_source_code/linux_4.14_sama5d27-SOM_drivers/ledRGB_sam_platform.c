...
/* PA10= RED, PB1 = GREEN, PA31 = BLUE */

/* Declare physical PA addresses */
static int PIOA_SODR1 = 0xFC038010;
static int PIOA_CODR1 = 0xFC038014;
static int PIOA_MSKR1 = 0xFC038000;
static int PIOA_CFGR1 = 0xFC038004;

/* Declare physical PB addresses */
static int PIOB_SODR1 = 0xFC038050;
static int PIOB_CODR1 = 0xFC038054;
static int PIOB_MSKR1 = 0xFC038040;
static int PIOB_CFGR1 = 0xFC038044;

/* Declare __iomem pointers that will keep virtual addresses */
static void __iomem *PIOA_SODR1_W;
static void __iomem *PIOA_CODR1_W;
static void __iomem *PIOA_MSKR1_V;
static void __iomem *PIOA_CFGR1_V;

static void __iomem *PIOB_SODR1_W;
static void __iomem *PIOB_CODR1_W;
static void __iomem *PIOB_MSKR1_V;
static void __iomem *PIOB_CFGR1_V;
						 ...
						 size_t count, loff_t *ppos)
{
	...
	/* compare strings to switch on/off the LED, PA10= RED, PB1 = GREEN, PA31 = BLUE */
	if(!strcmp(led_device->led_value, led_on)) {
		if(!strcmp(led_device->led_name, "ledred"))
			iowrite32(led_device->led_mask, PIOA_SODR1_W);
		else if(!strcmp(led_device->led_name, "ledgreen"))
			iowrite32(led_device->led_mask, PIOB_SODR1_W);
		...
		if(!strcmp(led_device->led_name, "ledred"))
			iowrite32(led_device->led_mask, PIOA_CODR1_W);
		else if(!strcmp(led_device->led_name, "ledgreen"))
			iowrite32(led_device->led_mask, PIOB_CODR1_W);
		else if(!strcmp(led_device->led_name, "ledblue"))
			iowrite32(led_device->led_mask, PIOA_CODR1_W);
		else {
			pr_info("Bad value\n");
...
static int __init led_probe(struct platform_device *pdev)
{
	...
	PIOA_MSKR1_V = devm_ioremap(&pdev->dev, PIOA_MSKR1, sizeof(u32));
	PIOA_SODR1_W = devm_ioremap(&pdev->dev, PIOA_SODR1, sizeof(u32));
	PIOA_CODR1_W = devm_ioremap(&pdev->dev, PIOA_CODR1, sizeof(u32));
	PIOA_CFGR1_V = devm_ioremap(&pdev->dev, PIOA_CFGR1, sizeof(u32));
	
	PIOB_MSKR1_V = devm_ioremap(&pdev->dev, PIOB_MSKR1, sizeof(u32));
	PIOB_SODR1_W = devm_ioremap(&pdev->dev, PIOB_SODR1, sizeof(u32));
	PIOB_CODR1_W = devm_ioremap(&pdev->dev, PIOB_CODR1, sizeof(u32));
	PIOB_CFGR1_V = devm_ioremap(&pdev->dev, PIOB_CFGR1, sizeof(u32));

	/* Initialize all the virtual registers */
	iowrite32(PIOA_MASK_ALL_LEDS, PIOA_MSKR1_V); /* Enable all leds */
	iowrite32(PIO_CFGR1_MASK, PIOA_CFGR1_V);     /* set enabled leds to output */
	iowrite32(PIOA_MASK_ALL_LEDS, PIOA_CODR1_W); /* Clear all the leds */	

	iowrite32(PIOB_MASK_ALL_LEDS, PIOB_MSKR1_V); /* Enable all leds */
	iowrite32(PIO_CFGR1_MASK, PIOB_CFGR1_V);     /* set enabled leds to output */
	iowrite32(PIOB_MASK_ALL_LEDS, PIOB_CODR1_W); /* Clear all the leds */	
	...
	/* Assigns a different mask for each led, PA10= RED, PB1 = GREEN, PA31 = BLUE */
...
static void led_exit(void)
{
	...
	iowrite32(PIOA_MASK_ALL_LEDS, PIOA_CODR1_W); 
	iowrite32(PIOB_MASK_ALL_LEDS, PIOB_CODR1_W); 	
