...
int
test_it5(struct kobject* kobj, char* buffer, loff_t off, size_t count, int val)
{
	printk("test_it5: count=%d, val=%d\n", count, val);
	return 0;
}
EXPORT_SYMBOL_GPL(test_it5);
...
/* following are the sysfs callback functions. All register read/write are
 * 16-bit wide */
...
static int tmp102_detect(struct i2c_adapter *adapter, int address, int kind)
{
	...
	if (kind < 0) {
		...
		if ((i2c_smbus_read_byte_data(new_client, 0) < 0))
			goto exit_kfree;
	}
